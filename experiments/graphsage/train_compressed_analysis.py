...
def run(proc_id, n_gpus, args, devices, data, compresser):
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, (input_nodes, seeds, blocks) in enumerate(dataloader):
            ...
            # print(batch_pred.norm(2, dim=1).mean().item())
            t5 = time.time()
    ...
    if proc_id == 0:
        with open("results/acc.txt", "a+") as f:
            print("GraphSAGE", args.mode, args.length, args.width, args.dataset, args.sparse, args.dist, args.fan_out,  float(best_eval), float(best_test), sep="\t", file=f)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--num-epochs', type=int, default=41)
    ...
    argparser.add_argument("--dist", type=str, default="random")                        
    argparser.add_argument("--sparse", type=float, default=1.0)                                  
    ...
    if args.dist == "random":
        perm = th.randperm(g.number_of_edges())
        removed_edges = perm[:int(g.number_of_edges() * (1-args.sparse))]
        if len(removed_edges) > 0:
            g.remove_edges(removed_edges)        
    elif args.dist == "high":
        bs = int(0.05 * g.number_of_edges())
        total_edges = g.number_of_edges()
        for i in range(0, int(total_edges*(1-args.sparse)), bs):
            size = min(int(total_edges*(1-args.sparse))-i, bs)

            g.ndata['deg'] = g.in_degrees().float()
            g.apply_edges(fn.u_mul_v('deg', 'deg', 'weight'))
            edge_weights = g.edata['weight'].float()
            removed_edges = edge_weights.topk(size, largest=False)[1]
            if len(removed_edges) > 0:
                g.remove_edges(removed_edges) 

    elif args.dist == "low":
        bs = int(0.02 * g.number_of_edges())
        total_edges = g.number_of_edges()
        for i in range(0, int(total_edges*(1-args.sparse)), bs):
            size = min(int(total_edges*(1-args.sparse))-i, bs)
            ...
            g.apply_edges(fn.u_mul_v('deg', 'deg', 'weight'))
            edge_weights = g.edata['weight'].float()
            print(len(edge_weights), g.number_of_edges(), size)
            removed_edges = edge_weights.topk(size, largest=True)[1]
            if len(removed_edges) > 0:
                g.remove_edges(removed_edges) 
