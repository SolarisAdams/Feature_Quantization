...
class SAINTSampler:
    ...
    def __init__(self, node_budget, dn, g, train_nid, num_workers_sampler, num_subg_sampler=10000,
        ...
        self.online = online
        self.full = full
        # print(self.train_g.num_nodes(), self.node_budget, math.ceil(self.train_g.num_nodes() / self.node_budget))
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            if self.online:
                ...
                ret = dgl.node_subgraph(self.train_g, subgraph)
                ret.edata.pop("_ID")
                return ret
            else:
                ret = dgl.node_subgraph(self.train_g, self.subgraphs[idx])
                ret.edata.pop("_ID")
                return ret
        else:
