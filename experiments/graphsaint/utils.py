...
def evaluate(model, g, labels, mask, multilabel=False, compresser=None, cpu_flag=False):
    model.eval()
    with torch.no_grad():
        h = compresser.decompress(g.ndata["features"], torch.cuda.current_device())
        # if cpu_flag:
        #     h = h.cpu()
        logits = model(g, h)
        logits = logits[mask]
...
def load_data(args, multilabel=None):
